...
class Subject(Build_Subject):
    ...
    def generate_design_matrix(
        ...
        iso_container = self.bids_container[
            f"run-{run}"
        ]  # BIDS container for the current run
        events = pd.read_csv(iso_container["event"], sep="\t")  # Load events file
        confounds = pd.read_csv(
            iso_container["confounds"], sep="\t"
        )  # Load fmriprep regressors
        ...
        events = first_level.make_first_level_design_matrix(
        ...
        if non_steady_state:
            confounds["non_steady_state"] = aggregate_non_steady_state(
                length=n_scans, dummy_value=dummy_value
            )
        ...
        if motion_outliers:
            confound_vars += [
                x for x in list(confounds.columns) if "motion_outlier" in x
            ]
...
\n\n
    ...
    def _run_contrast(
        self,
        glm: first_level.FirstLevelModel,
        contrast: str,
        title: str,
        output_type: str,
        smoothing: float,
        plot_brains: bool = False,
        plot_type: str = "stat",
    ):
        """
        ...
        if output_type == "condition":
            model_base = os.path.join(
                self.first_level_output, "models", "condition-maps"
            )
            plot_base = os.path.join(self.first_level_output, "plots", "condition-maps")

        elif output_type == "contrast":
            model_base = os.path.join(
                self.first_level_output, "models", "contrast-maps"
            )
            plot_base = os.path.join(self.first_level_output, "plots", "contrast-maps")

        else:
            raise ValueError(
                f"Output type {output_type} invalid - must be condition or contrast"
            )

        # Output name and path for computed contrast
        nifti_output = (
            f"sub-{self.sub_id}_{output_type}-{title}_smoothing-{kernel}mm_z-map.nii.gz"
        )
        ...
        if plot_brains:
            ...
            # Make GLM report
            make_glm_report(
                model=glm, contrasts=contrast, plot_type="glass"
            ).save_as_html(report_output)
    ...
    def run_first_level_glm(
        ...
        if user_design_matrices is not None:
            ...
            matrices = self.first_level_design(
                non_steady_state=non_steady_state,
                auto_block_regressors=auto_block_regressors,
                motion_outliers=motion_outliers,
                drop_fixation=drop_fixation,
        ...
        model = glm.fit(
            self.bids_container["all_preprocessed_bold"], design_matrices=matrices
        )
